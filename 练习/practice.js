...
// var numBusesToDestination = function(routes, source, target) {
//   // i为公交车，routes[i]为公交车路线，routes[i][j]为站点
//   if (source === target) {
//     return 0
//   }
//   const len = routes.length
//   // 哈希表保存能到每个车站的所有公交车，value值为数组
//   const map = new Map()
//   for (let i = 0; i < len; i++) {
//     for (const site of routes[i]) {
//       let list = (map.get(site) || [])
...
//     while (queLen--) {
//       // 出队，并得到经过该站点的所有公交车
//       const buses = map.get(queue.shift())
//       // 先遍历出队得到的所有公交车
...
//           crossed[bus] = true
//           // 遍历每辆公交车的所有站点
//           for (const site of routes[bus]) {
//             // 由于BFS的特性,等于目标的第一个搜索到的站点必然是最短换乘
//             if (site === target) {
//               return res
//             }
//             queue.push(site)
